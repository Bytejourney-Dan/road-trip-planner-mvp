...
export default function CalendarView({ 
  ...
  const weeksData = [];
  const daysPerWeek = 7;
  for (let i = 0; i < itinerary.days.length; i += daysPerWeek) {
    weeksData.push(itinerary.days.slice(i, i + daysPerWeek));
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-gray-600 text-sm">{itinerary.totalDays}-day journey • {itinerary.totalDistance} miles</p>
      </div>

      {/* Weekly Calendar Grid */}
      <div className="space-y-8">
        {weeksData.map((week, weekIndex) => (
          <div key={weekIndex} className="bg-white/20 backdrop-blur-sm rounded-xl p-4">
          ...
          const activeCustomAttractions = customDayAttractions.filter(custom => !custom.isRemoved);
                ...
                return (
                  <Card key={day.dayNumber} className="overflow-hidden h-fit">
                    <CardHeader className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-2">
                          <div className="text-center">
                            <div className="text-lg font-bold">{dayInfo.dayNumber}</div>
                            <div className="text-xs opacity-90">{dayInfo.month}</div>
                          </div>
                          <div>
                            <CardTitle className="text-sm text-white">Day {day.dayNumber}</CardTitle>
                            <p className="text-blue-100 text-xs">{dayInfo.dayName}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="flex items-center text-blue-100 mb-1">
                            <MapPin className="h-3 w-3 mr-1" />
                            <span className="text-xs">{day.route?.distance || 'N/A'} mi</span>
                          </div>
                          <div className="flex items-center text-blue-100">
                            <Clock className="h-3 w-3 mr-1" />
                            <span className="text-xs">{day.route?.drivingTime || 'N/A'}</span>
                          </div>
                        </div>
                      </div>
                    </CardHeader>
                    ...
                    <CardContent className="p-3">
                      ...
                      <div className="mb-3 p-2 bg-green-50 border border-green-200 rounded-lg">
                        <div className="flex items-center">
                          <Bed className="h-3 w-3 text-green-600 mr-1" />
                          <div>
                            <h4 className="font-medium text-gray-900 text-xs">Overnight</h4>
                            <p className="text-xs text-gray-700 truncate">{day.overnightLocation}</p>
                          </div>
                        </div>
                      </div>
                      ...
                      {day.route && (
                        <div className="mb-3 p-2 bg-blue-50 border border-blue-200 rounded-lg">
                          <h4 className="font-medium text-gray-900 mb-1 flex items-center text-xs">
                          ...
                          <div className="space-y-2">
                            ...
                            {filteredAttractions.map((attraction, attractionIndex) => {
                              ...
                              return (
                                <div 
                                  ...
                                  className="flex items-start justify-between p-2 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 transition-colors duration-200"
                                  data-testid={`calendar-attraction-${day.dayNumber}-${originalIndex}`}
                                >
                                  <div className="flex-1 min-w-0">
                                    <h5 className="font-medium text-gray-900 mb-1 text-xs truncate">
                                      {attraction.name}
                                    </h5>
                                  ...
                                  {onRemoveAttraction && (
                                    <button
                                      ...
                                      className="ml-2 p-1 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-all duration-200 flex-shrink-0"
                            ...
                            {activeCustomAttractions.map((customAttraction, customIndex) => (
                              <div 
                                ...
                                className="flex items-start justify-between p-2 bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 transition-colors duration-200"
                      ...
                      {filteredAttractions.length === 0 && activeCustomAttractions.length === 0 && (
                        <div className="text-center py-3 text-gray-500">
                          <Camera className="h-4 w-4 mx-auto mb-1 text-gray-300" />
                          <p className="text-xs">No attractions planned</p>
                        </div>
                      )}
                    </CardContent>
                  </Card>
